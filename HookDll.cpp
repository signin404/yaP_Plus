...
BOOL CreateProcessInternal(
    ...
    // [核心修复] 只要路径发生了变化（重定向了，或者被我们解析补全了）
    // 就必须重组命令行，并清空 AppName，确保 argv[0] 与实际执行文件一致！
    if (!redirectedExe.empty() || (wasResolved && lpApplicationName == NULL)) {
        ...
        // ！！！关键点：强制清空 AppName ！！！
        // 让 CreateProcess 完全依赖我们构造的正确命令行
        finalAppName = NULL;
