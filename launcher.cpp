...
// NOTE: AreWaitProcessesRunning is no longer needed and has been removed.
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
    ...
    if (isFirstInstance) {
        ...
        if (!CreateProcessW(NULL, commandLineBuffer, NULL, NULL, FALSE, 0, NULL, finalWorkDir.c_str(), &si, &pi)) {
            ...
            // We still need to wait for the main process to exit, but we do it inside the hybrid loop.
            // We close the thread handle as we don't need it. The process handle is used for waiting.
            CloseHandle(pi.hThread);
        }
        
        // --- HYBRID (EVENT-DRIVEN + TIMED RESCAN) WAIT LOGIC ---
        // This robustly waits for a "session" of processes to end, handling restarts and child processes.

        std::vector<std::wstring> sessionProcessNames;
        ...
        while (std::getline(waitStream, waitLine)) {
            ...
            if (delimiterPos != std::wstring::npos) {
                ...
                if (_wcsicmp(key.c_str(), L"waitprocess") == 0) {
                    ...
                    sessionProcessNames.push_back(ExpandVariables(value, variables));
                }
            }
        }
        if (GetValueFromIniContent(iniContent, L"General", L"multiple") == L"1") {
            const wchar_t* mainAppFilename = PathFindFileNameW(absoluteAppPath.c_str());
            if (mainAppFilename && wcslen(mainAppFilename) > 0) {
                sessionProcessNames.push_back(mainAppFilename);
            }
        }

        if (!sessionProcessNames.empty()) {
            std::wstring waitCheckStr = GetValueFromIniContent(iniContent, L"General", L"waitcheck");
            int waitCheckSeconds = waitCheckStr.empty() ? 10 : _wtoi(waitCheckStr.c_str());
            if (waitCheckSeconds <= 0) waitCheckSeconds = 10;
            DWORD waitTimeoutMs = waitCheckSeconds * 1000;

            Sleep(3000); // Initial delay to allow child processes to spawn.
            ...
            while (true) {
                ...
                if (currentHandles.empty()) {
                    // No matching processes found, the session has ended.
                    break; 
                }

                if (currentHandles.size() <= MAXIMUM_WAIT_OBJECTS) {
                    // Wait for ANY process to exit, or until the timeout.
                    // This is efficient and consumes no CPU while waiting.
                    WaitForMultipleObjects(
                        (DWORD)currentHandles.size(),
                        currentHandles.data(),
                        FALSE, // bWaitAll = FALSE: Wake up if ANY process exits.
                        waitTimeoutMs
                    );
                } else {
                    // Fallback for the rare case of having more than 64 processes to watch.
                    Sleep(waitTimeoutMs);
                }

                // Clean up handles from this iteration to prevent resource leaks.
                for (HANDLE h : currentHandles) {
                    CloseHandle(h);
                }
            }
        }
        
        // Close the handle to the initial process if it was created successfully
        if (pi.hProcess) {
            CloseHandle(pi.hProcess);
        }

        // --- END OF HYBRID WAIT LOGIC ---
