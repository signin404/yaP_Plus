...
std::wstring ToBase32StringSuitableForDirName(const std::vector<uint8_t>& buff) {
    ...
    while (i < l) {
        ...
        result += s_Base32Char[((b0 & 0xE0) >> 5) | ((b1 & 0x03) << 3)];
        result += s_Base32Char[(b1 >> 2) & 0x1F];
        result += s_Base32Char[((b1 & 0xC0) >> 6) | ((b2 & 0x0F) << 2)];
        result += s_Base32Char[(b2 >> 4) & 0x1F];
        result += s_Base32Char[((b2 & 0xF0) >> 4) | ((b3 & 0x01) << 4)];
        result += s_Base32Char[(b3 >> 1) & 0x1F];
        result += s_Base32Char[((b3 & 0xFE) >> 1) | ((b4 & 0x07) << 4)];
        result += s_Base32Char[(b4 >> 3) & 0x1F];
    }
    return result;
}
...
std::wstring CalculateNetPath(std::wstring absoluteAppPath, int mode) {
    if (mode < 0 || mode >= 960) return L"";

    int m = mode;
    int path_ext_mode = m % 2; m /= 2;  // 0:无扩展名, 1:带扩展名
    int case_mode     = m % 3; m /= 3;  // 0:原样, 1:全大写, 2:全小写
    int proto_mode    = m % 4; m /= 4;  // 0:无, 1:file:///, 2:file://, 3:file:
    int slash_mode    = m % 2; m /= 2;  // 0:反斜杠, 1:正斜杠
    int salt_mode     = m % 5; m /= 5;  // 0:无, 1:后置File:, 2:前置File:, 3:后置Url:, 4:后置file:
    int enc_mode      = m % 4; m /= 4;  // 0:纯UTF8, 1:7bit+UTF8, 2:NRBF+7bit+UTF8, 3:纯UTF16LE

    // 1. 提取前缀文件名（永远不带扩展名，对应 .NET 10 行为）
    ...
    if (dotPos != std::wstring::npos && (slashPos == std::wstring::npos || dotPos > slashPos)) {
        if (path_ext_mode == 0) pathStr = pathStr.substr(0, dotPos);
    }

    // 3. 处理协议前缀
    std::wstring uri;
    if (proto_mode == 0) uri = pathStr;
    else if (proto_mode == 1) uri = L"file:///" + pathStr;
    else if (proto_mode == 2) uri = L"file://" + pathStr;
    else uri = L"file:" + pathStr;
    ...
    // 9. 处理盐值
    std::string salt = "";
    if (salt_mode == 1 || salt_mode == 2) salt = "File:";
    else if (salt_mode == 3) salt = "Url:";
    else if (salt_mode == 4) salt = "file:";
    ...
    // 后置盐
    if (salt_mode == 1 || salt_mode == 3 || salt_mode == 4) {
        rawData.insert(rawData.end(), salt.begin(), salt.end());
    }
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
    ...
    if (isFirstInstance) {
        ...
        variables[L"NETHASH256"] = CalculateNetPath(absoluteAppPath, 256);
        variables[L"NETHASH257"] = CalculateNetPath(absoluteAppPath, 257);
        variables[L"NETHASH258"] = CalculateNetPath(absoluteAppPath, 258);
        variables[L"NETHASH259"] = CalculateNetPath(absoluteAppPath, 259);
        variables[L"NETHASH260"] = CalculateNetPath(absoluteAppPath, 260);
        variables[L"NETHASH261"] = CalculateNetPath(absoluteAppPath, 261);
        variables[L"NETHASH262"] = CalculateNetPath(absoluteAppPath, 262);
        variables[L"NETHASH263"] = CalculateNetPath(absoluteAppPath, 263);
        variables[L"NETHASH264"] = CalculateNetPath(absoluteAppPath, 264);
        variables[L"NETHASH265"] = CalculateNetPath(absoluteAppPath, 265);
        variables[L"NETHASH266"] = CalculateNetPath(absoluteAppPath, 266);
        variables[L"NETHASH267"] = CalculateNetPath(absoluteAppPath, 267);
        variables[L"NETHASH268"] = CalculateNetPath(absoluteAppPath, 268);
        variables[L"NETHASH269"] = CalculateNetPath(absoluteAppPath, 269);
        variables[L"NETHASH270"] = CalculateNetPath(absoluteAppPath, 270);
        variables[L"NETHASH271"] = CalculateNetPath(absoluteAppPath, 271);
        variables[L"NETHASH272"] = CalculateNetPath(absoluteAppPath, 272);
        variables[L"NETHASH273"] = CalculateNetPath(absoluteAppPath, 273);
        variables[L"NETHASH274"] = CalculateNetPath(absoluteAppPath, 274);
        variables[L"NETHASH275"] = CalculateNetPath(absoluteAppPath, 275);
        variables[L"NETHASH276"] = CalculateNetPath(absoluteAppPath, 276);
        variables[L"NETHASH277"] = CalculateNetPath(absoluteAppPath, 277);
        variables[L"NETHASH278"] = CalculateNetPath(absoluteAppPath, 278);
        variables[L"NETHASH279"] = CalculateNetPath(absoluteAppPath, 279);
        variables[L"NETHASH280"] = CalculateNetPath(absoluteAppPath, 280);
        variables[L"NETHASH281"] = CalculateNetPath(absoluteAppPath, 281);
        variables[L"NETHASH282"] = CalculateNetPath(absoluteAppPath, 282);
        variables[L"NETHASH283"] = CalculateNetPath(absoluteAppPath, 283);
        variables[L"NETHASH284"] = CalculateNetPath(absoluteAppPath, 284);
        variables[L"NETHASH285"] = CalculateNetPath(absoluteAppPath, 285);
        variables[L"NETHASH286"] = CalculateNetPath(absoluteAppPath, 286);
        variables[L"NETHASH287"] = CalculateNetPath(absoluteAppPath, 287);
        variables[L"NETHASH288"] = CalculateNetPath(absoluteAppPath, 288);
        variables[L"NETHASH289"] = CalculateNetPath(absoluteAppPath, 289);
        variables[L"NETHASH290"] = CalculateNetPath(absoluteAppPath, 290);
        variables[L"NETHASH291"] = CalculateNetPath(absoluteAppPath, 291);
        variables[L"NETHASH292"] = CalculateNetPath(absoluteAppPath, 292);
        variables[L"NETHASH293"] = CalculateNetPath(absoluteAppPath, 293);
        variables[L"NETHASH294"] = CalculateNetPath(absoluteAppPath, 294);
        variables[L"NETHASH295"] = CalculateNetPath(absoluteAppPath, 295);
        variables[L"NETHASH296"] = CalculateNetPath(absoluteAppPath, 296);
        variables[L"NETHASH297"] = CalculateNetPath(absoluteAppPath, 297);
        variables[L"NETHASH298"] = CalculateNetPath(absoluteAppPath, 298);
        variables[L"NETHASH299"] = CalculateNetPath(absoluteAppPath, 299);
        variables[L"NETHASH300"] = CalculateNetPath(absoluteAppPath, 300);
        variables[L"NETHASH301"] = CalculateNetPath(absoluteAppPath, 301);
        variables[L"NETHASH302"] = CalculateNetPath(absoluteAppPath, 302);
        variables[L"NETHASH303"] = CalculateNetPath(absoluteAppPath, 303);
        variables[L"NETHASH304"] = CalculateNetPath(absoluteAppPath, 304);
        variables[L"NETHASH305"] = CalculateNetPath(absoluteAppPath, 305);
        variables[L"NETHASH306"] = CalculateNetPath(absoluteAppPath, 306);
        variables[L"NETHASH307"] = CalculateNetPath(absoluteAppPath, 307);
        variables[L"NETHASH308"] = CalculateNetPath(absoluteAppPath, 308);
        variables[L"NETHASH309"] = CalculateNetPath(absoluteAppPath, 309);
        variables[L"NETHASH310"] = CalculateNetPath(absoluteAppPath, 310);
        variables[L"NETHASH311"] = CalculateNetPath(absoluteAppPath, 311);
        variables[L"NETHASH312"] = CalculateNetPath(absoluteAppPath, 312);
        variables[L"NETHASH313"] = CalculateNetPath(absoluteAppPath, 313);
        variables[L"NETHASH314"] = CalculateNetPath(absoluteAppPath, 314);
        variables[L"NETHASH315"] = CalculateNetPath(absoluteAppPath, 315);
        variables[L"NETHASH316"] = CalculateNetPath(absoluteAppPath, 316);
        variables[L"NETHASH317"] = CalculateNetPath(absoluteAppPath, 317);
        variables[L"NETHASH318"] = CalculateNetPath(absoluteAppPath, 318);
        variables[L"NETHASH319"] = CalculateNetPath(absoluteAppPath, 319);
        variables[L"NETHASH320"] = CalculateNetPath(absoluteAppPath, 320);
        variables[L"NETHASH321"] = CalculateNetPath(absoluteAppPath, 321);
        variables[L"NETHASH322"] = CalculateNetPath(absoluteAppPath, 322);
        variables[L"NETHASH323"] = CalculateNetPath(absoluteAppPath, 323);
        variables[L"NETHASH324"] = CalculateNetPath(absoluteAppPath, 324);
        variables[L"NETHASH325"] = CalculateNetPath(absoluteAppPath, 325);
        variables[L"NETHASH326"] = CalculateNetPath(absoluteAppPath, 326);
        variables[L"NETHASH327"] = CalculateNetPath(absoluteAppPath, 327);
        variables[L"NETHASH328"] = CalculateNetPath(absoluteAppPath, 328);
        variables[L"NETHASH329"] = CalculateNetPath(absoluteAppPath, 329);
        variables[L"NETHASH330"] = CalculateNetPath(absoluteAppPath, 330);
        variables[L"NETHASH331"] = CalculateNetPath(absoluteAppPath, 331);
        variables[L"NETHASH332"] = CalculateNetPath(absoluteAppPath, 332);
        variables[L"NETHASH333"] = CalculateNetPath(absoluteAppPath, 333);
        variables[L"NETHASH334"] = CalculateNetPath(absoluteAppPath, 334);
        variables[L"NETHASH335"] = CalculateNetPath(absoluteAppPath, 335);
        variables[L"NETHASH336"] = CalculateNetPath(absoluteAppPath, 336);
        variables[L"NETHASH337"] = CalculateNetPath(absoluteAppPath, 337);
        variables[L"NETHASH338"] = CalculateNetPath(absoluteAppPath, 338);
        variables[L"NETHASH339"] = CalculateNetPath(absoluteAppPath, 339);
        variables[L"NETHASH340"] = CalculateNetPath(absoluteAppPath, 340);
        variables[L"NETHASH341"] = CalculateNetPath(absoluteAppPath, 341);
        variables[L"NETHASH342"] = CalculateNetPath(absoluteAppPath, 342);
        variables[L"NETHASH343"] = CalculateNetPath(absoluteAppPath, 343);
        variables[L"NETHASH344"] = CalculateNetPath(absoluteAppPath, 344);
        variables[L"NETHASH345"] = CalculateNetPath(absoluteAppPath, 345);
        variables[L"NETHASH346"] = CalculateNetPath(absoluteAppPath, 346);
        variables[L"NETHASH347"] = CalculateNetPath(absoluteAppPath, 347);
        variables[L"NETHASH348"] = CalculateNetPath(absoluteAppPath, 348);
        variables[L"NETHASH349"] = CalculateNetPath(absoluteAppPath, 349);
        variables[L"NETHASH350"] = CalculateNetPath(absoluteAppPath, 350);
        variables[L"NETHASH351"] = CalculateNetPath(absoluteAppPath, 351);
        variables[L"NETHASH352"] = CalculateNetPath(absoluteAppPath, 352);
        variables[L"NETHASH353"] = CalculateNetPath(absoluteAppPath, 353);
        variables[L"NETHASH354"] = CalculateNetPath(absoluteAppPath, 354);
        variables[L"NETHASH355"] = CalculateNetPath(absoluteAppPath, 355);
        variables[L"NETHASH356"] = CalculateNetPath(absoluteAppPath, 356);
        variables[L"NETHASH357"] = CalculateNetPath(absoluteAppPath, 357);
        variables[L"NETHASH358"] = CalculateNetPath(absoluteAppPath, 358);
        variables[L"NETHASH359"] = CalculateNetPath(absoluteAppPath, 359);
        variables[L"NETHASH360"] = CalculateNetPath(absoluteAppPath, 360);
        variables[L"NETHASH361"] = CalculateNetPath(absoluteAppPath, 361);
        variables[L"NETHASH362"] = CalculateNetPath(absoluteAppPath, 362);
        variables[L"NETHASH363"] = CalculateNetPath(absoluteAppPath, 363);
        variables[L"NETHASH364"] = CalculateNetPath(absoluteAppPath, 364);
        variables[L"NETHASH365"] = CalculateNetPath(absoluteAppPath, 365);
        variables[L"NETHASH366"] = CalculateNetPath(absoluteAppPath, 366);
        variables[L"NETHASH367"] = CalculateNetPath(absoluteAppPath, 367);
        variables[L"NETHASH368"] = CalculateNetPath(absoluteAppPath, 368);
        variables[L"NETHASH369"] = CalculateNetPath(absoluteAppPath, 369);
        variables[L"NETHASH370"] = CalculateNetPath(absoluteAppPath, 370);
        variables[L"NETHASH371"] = CalculateNetPath(absoluteAppPath, 371);
        variables[L"NETHASH372"] = CalculateNetPath(absoluteAppPath, 372);
        variables[L"NETHASH373"] = CalculateNetPath(absoluteAppPath, 373);
        variables[L"NETHASH374"] = CalculateNetPath(absoluteAppPath, 374);
        variables[L"NETHASH375"] = CalculateNetPath(absoluteAppPath, 375);
        variables[L"NETHASH376"] = CalculateNetPath(absoluteAppPath, 376);
        variables[L"NETHASH377"] = CalculateNetPath(absoluteAppPath, 377);
        variables[L"NETHASH378"] = CalculateNetPath(absoluteAppPath, 378);
        variables[L"NETHASH379"] = CalculateNetPath(absoluteAppPath, 379);
        variables[L"NETHASH380"] = CalculateNetPath(absoluteAppPath, 380);
        variables[L"NETHASH381"] = CalculateNetPath(absoluteAppPath, 381);
        variables[L"NETHASH382"] = CalculateNetPath(absoluteAppPath, 382);
        variables[L"NETHASH383"] = CalculateNetPath(absoluteAppPath, 383);
        variables[L"NETHASH384"] = CalculateNetPath(absoluteAppPath, 384);
        variables[L"NETHASH385"] = CalculateNetPath(absoluteAppPath, 385);
        variables[L"NETHASH386"] = CalculateNetPath(absoluteAppPath, 386);
        variables[L"NETHASH387"] = CalculateNetPath(absoluteAppPath, 387);
        variables[L"NETHASH388"] = CalculateNetPath(absoluteAppPath, 388);
        variables[L"NETHASH389"] = CalculateNetPath(absoluteAppPath, 389);
        variables[L"NETHASH390"] = CalculateNetPath(absoluteAppPath, 390);
        variables[L"NETHASH391"] = CalculateNetPath(absoluteAppPath, 391);
        variables[L"NETHASH392"] = CalculateNetPath(absoluteAppPath, 392);
        variables[L"NETHASH393"] = CalculateNetPath(absoluteAppPath, 393);
        variables[L"NETHASH394"] = CalculateNetPath(absoluteAppPath, 394);
        variables[L"NETHASH395"] = CalculateNetPath(absoluteAppPath, 395);
        variables[L"NETHASH396"] = CalculateNetPath(absoluteAppPath, 396);
        variables[L"NETHASH397"] = CalculateNetPath(absoluteAppPath, 397);
        variables[L"NETHASH398"] = CalculateNetPath(absoluteAppPath, 398);
        variables[L"NETHASH399"] = CalculateNetPath(absoluteAppPath, 399);
        variables[L"NETHASH400"] = CalculateNetPath(absoluteAppPath, 400);
        variables[L"NETHASH401"] = CalculateNetPath(absoluteAppPath, 401);
        variables[L"NETHASH402"] = CalculateNetPath(absoluteAppPath, 402);
        variables[L"NETHASH403"] = CalculateNetPath(absoluteAppPath, 403);
        variables[L"NETHASH404"] = CalculateNetPath(absoluteAppPath, 404);
        variables[L"NETHASH405"] = CalculateNetPath(absoluteAppPath, 405);
        variables[L"NETHASH406"] = CalculateNetPath(absoluteAppPath, 406);
        variables[L"NETHASH407"] = CalculateNetPath(absoluteAppPath, 407);
        variables[L"NETHASH408"] = CalculateNetPath(absoluteAppPath, 408);
        variables[L"NETHASH409"] = CalculateNetPath(absoluteAppPath, 409);
        variables[L"NETHASH410"] = CalculateNetPath(absoluteAppPath, 410);
        variables[L"NETHASH411"] = CalculateNetPath(absoluteAppPath, 411);
        variables[L"NETHASH412"] = CalculateNetPath(absoluteAppPath, 412);
        variables[L"NETHASH413"] = CalculateNetPath(absoluteAppPath, 413);
        variables[L"NETHASH414"] = CalculateNetPath(absoluteAppPath, 414);
        variables[L"NETHASH415"] = CalculateNetPath(absoluteAppPath, 415);
        variables[L"NETHASH416"] = CalculateNetPath(absoluteAppPath, 416);
        variables[L"NETHASH417"] = CalculateNetPath(absoluteAppPath, 417);
        variables[L"NETHASH418"] = CalculateNetPath(absoluteAppPath, 418);
        variables[L"NETHASH419"] = CalculateNetPath(absoluteAppPath, 419);
        variables[L"NETHASH420"] = CalculateNetPath(absoluteAppPath, 420);
        variables[L"NETHASH421"] = CalculateNetPath(absoluteAppPath, 421);
        variables[L"NETHASH422"] = CalculateNetPath(absoluteAppPath, 422);
        variables[L"NETHASH423"] = CalculateNetPath(absoluteAppPath, 423);
        variables[L"NETHASH424"] = CalculateNetPath(absoluteAppPath, 424);
        variables[L"NETHASH425"] = CalculateNetPath(absoluteAppPath, 425);
        variables[L"NETHASH426"] = CalculateNetPath(absoluteAppPath, 426);
        variables[L"NETHASH427"] = CalculateNetPath(absoluteAppPath, 427);
        variables[L"NETHASH428"] = CalculateNetPath(absoluteAppPath, 428);
        variables[L"NETHASH429"] = CalculateNetPath(absoluteAppPath, 429);
        variables[L"NETHASH430"] = CalculateNetPath(absoluteAppPath, 430);
        variables[L"NETHASH431"] = CalculateNetPath(absoluteAppPath, 431);
        variables[L"NETHASH432"] = CalculateNetPath(absoluteAppPath, 432);
        variables[L"NETHASH433"] = CalculateNetPath(absoluteAppPath, 433);
        variables[L"NETHASH434"] = CalculateNetPath(absoluteAppPath, 434);
        variables[L"NETHASH435"] = CalculateNetPath(absoluteAppPath, 435);
        variables[L"NETHASH436"] = CalculateNetPath(absoluteAppPath, 436);
        variables[L"NETHASH437"] = CalculateNetPath(absoluteAppPath, 437);
        variables[L"NETHASH438"] = CalculateNetPath(absoluteAppPath, 438);
        variables[L"NETHASH439"] = CalculateNetPath(absoluteAppPath, 439);
        variables[L"NETHASH440"] = CalculateNetPath(absoluteAppPath, 440);
        variables[L"NETHASH441"] = CalculateNetPath(absoluteAppPath, 441);
        variables[L"NETHASH442"] = CalculateNetPath(absoluteAppPath, 442);
        variables[L"NETHASH443"] = CalculateNetPath(absoluteAppPath, 443);
        variables[L"NETHASH444"] = CalculateNetPath(absoluteAppPath, 444);
        variables[L"NETHASH445"] = CalculateNetPath(absoluteAppPath, 445);
        variables[L"NETHASH446"] = CalculateNetPath(absoluteAppPath, 446);
        variables[L"NETHASH447"] = CalculateNetPath(absoluteAppPath, 447);
        variables[L"NETHASH448"] = CalculateNetPath(absoluteAppPath, 448);
        variables[L"NETHASH449"] = CalculateNetPath(absoluteAppPath, 449);
        variables[L"NETHASH450"] = CalculateNetPath(absoluteAppPath, 450);
        variables[L"NETHASH451"] = CalculateNetPath(absoluteAppPath, 451);
        variables[L"NETHASH452"] = CalculateNetPath(absoluteAppPath, 452);
        variables[L"NETHASH453"] = CalculateNetPath(absoluteAppPath, 453);
        variables[L"NETHASH454"] = CalculateNetPath(absoluteAppPath, 454);
        variables[L"NETHASH455"] = CalculateNetPath(absoluteAppPath, 455);
        variables[L"NETHASH456"] = CalculateNetPath(absoluteAppPath, 456);
        variables[L"NETHASH457"] = CalculateNetPath(absoluteAppPath, 457);
        variables[L"NETHASH458"] = CalculateNetPath(absoluteAppPath, 458);
        variables[L"NETHASH459"] = CalculateNetPath(absoluteAppPath, 459);
        variables[L"NETHASH460"] = CalculateNetPath(absoluteAppPath, 460);
        variables[L"NETHASH461"] = CalculateNetPath(absoluteAppPath, 461);
        variables[L"NETHASH462"] = CalculateNetPath(absoluteAppPath, 462);
        variables[L"NETHASH463"] = CalculateNetPath(absoluteAppPath, 463);
        variables[L"NETHASH464"] = CalculateNetPath(absoluteAppPath, 464);
        variables[L"NETHASH465"] = CalculateNetPath(absoluteAppPath, 465);
        variables[L"NETHASH466"] = CalculateNetPath(absoluteAppPath, 466);
        variables[L"NETHASH467"] = CalculateNetPath(absoluteAppPath, 467);
        variables[L"NETHASH468"] = CalculateNetPath(absoluteAppPath, 468);
        variables[L"NETHASH469"] = CalculateNetPath(absoluteAppPath, 469);
        variables[L"NETHASH470"] = CalculateNetPath(absoluteAppPath, 470);
        variables[L"NETHASH471"] = CalculateNetPath(absoluteAppPath, 471);
        variables[L"NETHASH472"] = CalculateNetPath(absoluteAppPath, 472);
        variables[L"NETHASH473"] = CalculateNetPath(absoluteAppPath, 473);
        variables[L"NETHASH474"] = CalculateNetPath(absoluteAppPath, 474);
        variables[L"NETHASH475"] = CalculateNetPath(absoluteAppPath, 475);
        variables[L"NETHASH476"] = CalculateNetPath(absoluteAppPath, 476);
        variables[L"NETHASH477"] = CalculateNetPath(absoluteAppPath, 477);
        variables[L"NETHASH478"] = CalculateNetPath(absoluteAppPath, 478);
        variables[L"NETHASH479"] = CalculateNetPath(absoluteAppPath, 479);
        variables[L"NETHASH480"] = CalculateNetPath(absoluteAppPath, 480);
        variables[L"NETHASH481"] = CalculateNetPath(absoluteAppPath, 481);
        variables[L"NETHASH482"] = CalculateNetPath(absoluteAppPath, 482);
        variables[L"NETHASH483"] = CalculateNetPath(absoluteAppPath, 483);
        variables[L"NETHASH484"] = CalculateNetPath(absoluteAppPath, 484);
        variables[L"NETHASH485"] = CalculateNetPath(absoluteAppPath, 485);
        variables[L"NETHASH486"] = CalculateNetPath(absoluteAppPath, 486);
        variables[L"NETHASH487"] = CalculateNetPath(absoluteAppPath, 487);
        variables[L"NETHASH488"] = CalculateNetPath(absoluteAppPath, 488);
        variables[L"NETHASH489"] = CalculateNetPath(absoluteAppPath, 489);
        variables[L"NETHASH490"] = CalculateNetPath(absoluteAppPath, 490);
        variables[L"NETHASH491"] = CalculateNetPath(absoluteAppPath, 491);
        variables[L"NETHASH492"] = CalculateNetPath(absoluteAppPath, 492);
        variables[L"NETHASH493"] = CalculateNetPath(absoluteAppPath, 493);
        variables[L"NETHASH494"] = CalculateNetPath(absoluteAppPath, 494);
        variables[L"NETHASH495"] = CalculateNetPath(absoluteAppPath, 495);
        variables[L"NETHASH496"] = CalculateNetPath(absoluteAppPath, 496);
        variables[L"NETHASH497"] = CalculateNetPath(absoluteAppPath, 497);
        variables[L"NETHASH498"] = CalculateNetPath(absoluteAppPath, 498);
        variables[L"NETHASH499"] = CalculateNetPath(absoluteAppPath, 499);
        variables[L"NETHASH500"] = CalculateNetPath(absoluteAppPath, 500);
        variables[L"NETHASH501"] = CalculateNetPath(absoluteAppPath, 501);
        variables[L"NETHASH502"] = CalculateNetPath(absoluteAppPath, 502);
        variables[L"NETHASH503"] = CalculateNetPath(absoluteAppPath, 503);
        variables[L"NETHASH504"] = CalculateNetPath(absoluteAppPath, 504);
        variables[L"NETHASH505"] = CalculateNetPath(absoluteAppPath, 505);
        variables[L"NETHASH506"] = CalculateNetPath(absoluteAppPath, 506);
        variables[L"NETHASH507"] = CalculateNetPath(absoluteAppPath, 507);
        variables[L"NETHASH508"] = CalculateNetPath(absoluteAppPath, 508);
        variables[L"NETHASH509"] = CalculateNetPath(absoluteAppPath, 509);
        variables[L"NETHASH510"] = CalculateNetPath(absoluteAppPath, 510);
        variables[L"NETHASH511"] = CalculateNetPath(absoluteAppPath, 511);
        variables[L"NETHASH512"] = CalculateNetPath(absoluteAppPath, 512);
        variables[L"NETHASH513"] = CalculateNetPath(absoluteAppPath, 513);
        variables[L"NETHASH514"] = CalculateNetPath(absoluteAppPath, 514);
        variables[L"NETHASH515"] = CalculateNetPath(absoluteAppPath, 515);
        variables[L"NETHASH516"] = CalculateNetPath(absoluteAppPath, 516);
        variables[L"NETHASH517"] = CalculateNetPath(absoluteAppPath, 517);
        variables[L"NETHASH518"] = CalculateNetPath(absoluteAppPath, 518);
        variables[L"NETHASH519"] = CalculateNetPath(absoluteAppPath, 519);
        variables[L"NETHASH520"] = CalculateNetPath(absoluteAppPath, 520);
        variables[L"NETHASH521"] = CalculateNetPath(absoluteAppPath, 521);
        variables[L"NETHASH522"] = CalculateNetPath(absoluteAppPath, 522);
        variables[L"NETHASH523"] = CalculateNetPath(absoluteAppPath, 523);
        variables[L"NETHASH524"] = CalculateNetPath(absoluteAppPath, 524);
        variables[L"NETHASH525"] = CalculateNetPath(absoluteAppPath, 525);
        variables[L"NETHASH526"] = CalculateNetPath(absoluteAppPath, 526);
        variables[L"NETHASH527"] = CalculateNetPath(absoluteAppPath, 527);
        variables[L"NETHASH528"] = CalculateNetPath(absoluteAppPath, 528);
        variables[L"NETHASH529"] = CalculateNetPath(absoluteAppPath, 529);
        variables[L"NETHASH530"] = CalculateNetPath(absoluteAppPath, 530);
        variables[L"NETHASH531"] = CalculateNetPath(absoluteAppPath, 531);
        variables[L"NETHASH532"] = CalculateNetPath(absoluteAppPath, 532);
        variables[L"NETHASH533"] = CalculateNetPath(absoluteAppPath, 533);
        variables[L"NETHASH534"] = CalculateNetPath(absoluteAppPath, 534);
        variables[L"NETHASH535"] = CalculateNetPath(absoluteAppPath, 535);
        variables[L"NETHASH536"] = CalculateNetPath(absoluteAppPath, 536);
        variables[L"NETHASH537"] = CalculateNetPath(absoluteAppPath, 537);
        variables[L"NETHASH538"] = CalculateNetPath(absoluteAppPath, 538);
        variables[L"NETHASH539"] = CalculateNetPath(absoluteAppPath, 539);
        variables[L"NETHASH540"] = CalculateNetPath(absoluteAppPath, 540);
        variables[L"NETHASH541"] = CalculateNetPath(absoluteAppPath, 541);
        variables[L"NETHASH542"] = CalculateNetPath(absoluteAppPath, 542);
        variables[L"NETHASH543"] = CalculateNetPath(absoluteAppPath, 543);
        variables[L"NETHASH544"] = CalculateNetPath(absoluteAppPath, 544);
        variables[L"NETHASH545"] = CalculateNetPath(absoluteAppPath, 545);
        variables[L"NETHASH546"] = CalculateNetPath(absoluteAppPath, 546);
        variables[L"NETHASH547"] = CalculateNetPath(absoluteAppPath, 547);
        variables[L"NETHASH548"] = CalculateNetPath(absoluteAppPath, 548);
        variables[L"NETHASH549"] = CalculateNetPath(absoluteAppPath, 549);
        variables[L"NETHASH550"] = CalculateNetPath(absoluteAppPath, 550);
        variables[L"NETHASH551"] = CalculateNetPath(absoluteAppPath, 551);
        variables[L"NETHASH552"] = CalculateNetPath(absoluteAppPath, 552);
        variables[L"NETHASH553"] = CalculateNetPath(absoluteAppPath, 553);
        variables[L"NETHASH554"] = CalculateNetPath(absoluteAppPath, 554);
        variables[L"NETHASH555"] = CalculateNetPath(absoluteAppPath, 555);
        variables[L"NETHASH556"] = CalculateNetPath(absoluteAppPath, 556);
        variables[L"NETHASH557"] = CalculateNetPath(absoluteAppPath, 557);
        variables[L"NETHASH558"] = CalculateNetPath(absoluteAppPath, 558);
        variables[L"NETHASH559"] = CalculateNetPath(absoluteAppPath, 559);
        variables[L"NETHASH560"] = CalculateNetPath(absoluteAppPath, 560);
        variables[L"NETHASH561"] = CalculateNetPath(absoluteAppPath, 561);
        variables[L"NETHASH562"] = CalculateNetPath(absoluteAppPath, 562);
        variables[L"NETHASH563"] = CalculateNetPath(absoluteAppPath, 563);
        variables[L"NETHASH564"] = CalculateNetPath(absoluteAppPath, 564);
        variables[L"NETHASH565"] = CalculateNetPath(absoluteAppPath, 565);
        variables[L"NETHASH566"] = CalculateNetPath(absoluteAppPath, 566);
        variables[L"NETHASH567"] = CalculateNetPath(absoluteAppPath, 567);
        variables[L"NETHASH568"] = CalculateNetPath(absoluteAppPath, 568);
        variables[L"NETHASH569"] = CalculateNetPath(absoluteAppPath, 569);
        variables[L"NETHASH570"] = CalculateNetPath(absoluteAppPath, 570);
        variables[L"NETHASH571"] = CalculateNetPath(absoluteAppPath, 571);
        variables[L"NETHASH572"] = CalculateNetPath(absoluteAppPath, 572);
        variables[L"NETHASH573"] = CalculateNetPath(absoluteAppPath, 573);
        variables[L"NETHASH574"] = CalculateNetPath(absoluteAppPath, 574);
        variables[L"NETHASH575"] = CalculateNetPath(absoluteAppPath, 575);
        variables[L"NETHASH576"] = CalculateNetPath(absoluteAppPath, 576);
        variables[L"NETHASH577"] = CalculateNetPath(absoluteAppPath, 577);
        variables[L"NETHASH578"] = CalculateNetPath(absoluteAppPath, 578);
        variables[L"NETHASH579"] = CalculateNetPath(absoluteAppPath, 579);
        variables[L"NETHASH580"] = CalculateNetPath(absoluteAppPath, 580);
        variables[L"NETHASH581"] = CalculateNetPath(absoluteAppPath, 581);
        variables[L"NETHASH582"] = CalculateNetPath(absoluteAppPath, 582);
        variables[L"NETHASH583"] = CalculateNetPath(absoluteAppPath, 583);
        variables[L"NETHASH584"] = CalculateNetPath(absoluteAppPath, 584);
        variables[L"NETHASH585"] = CalculateNetPath(absoluteAppPath, 585);
        variables[L"NETHASH586"] = CalculateNetPath(absoluteAppPath, 586);
        variables[L"NETHASH587"] = CalculateNetPath(absoluteAppPath, 587);
        variables[L"NETHASH588"] = CalculateNetPath(absoluteAppPath, 588);
        variables[L"NETHASH589"] = CalculateNetPath(absoluteAppPath, 589);
        variables[L"NETHASH590"] = CalculateNetPath(absoluteAppPath, 590);
        variables[L"NETHASH591"] = CalculateNetPath(absoluteAppPath, 591);
        variables[L"NETHASH592"] = CalculateNetPath(absoluteAppPath, 592);
        variables[L"NETHASH593"] = CalculateNetPath(absoluteAppPath, 593);
        variables[L"NETHASH594"] = CalculateNetPath(absoluteAppPath, 594);
        variables[L"NETHASH595"] = CalculateNetPath(absoluteAppPath, 595);
        variables[L"NETHASH596"] = CalculateNetPath(absoluteAppPath, 596);
        variables[L"NETHASH597"] = CalculateNetPath(absoluteAppPath, 597);
        variables[L"NETHASH598"] = CalculateNetPath(absoluteAppPath, 598);
        variables[L"NETHASH599"] = CalculateNetPath(absoluteAppPath, 599);
        variables[L"NETHASH600"] = CalculateNetPath(absoluteAppPath, 600);
        variables[L"NETHASH601"] = CalculateNetPath(absoluteAppPath, 601);
        variables[L"NETHASH602"] = CalculateNetPath(absoluteAppPath, 602);
        variables[L"NETHASH603"] = CalculateNetPath(absoluteAppPath, 603);
        variables[L"NETHASH604"] = CalculateNetPath(absoluteAppPath, 604);
        variables[L"NETHASH605"] = CalculateNetPath(absoluteAppPath, 605);
        variables[L"NETHASH606"] = CalculateNetPath(absoluteAppPath, 606);
        variables[L"NETHASH607"] = CalculateNetPath(absoluteAppPath, 607);
        variables[L"NETHASH608"] = CalculateNetPath(absoluteAppPath, 608);
        variables[L"NETHASH609"] = CalculateNetPath(absoluteAppPath, 609);
        variables[L"NETHASH610"] = CalculateNetPath(absoluteAppPath, 610);
        variables[L"NETHASH611"] = CalculateNetPath(absoluteAppPath, 611);
        variables[L"NETHASH612"] = CalculateNetPath(absoluteAppPath, 612);
        variables[L"NETHASH613"] = CalculateNetPath(absoluteAppPath, 613);
        variables[L"NETHASH614"] = CalculateNetPath(absoluteAppPath, 614);
        variables[L"NETHASH615"] = CalculateNetPath(absoluteAppPath, 615);
        variables[L"NETHASH616"] = CalculateNetPath(absoluteAppPath, 616);
        variables[L"NETHASH617"] = CalculateNetPath(absoluteAppPath, 617);
        variables[L"NETHASH618"] = CalculateNetPath(absoluteAppPath, 618);
        variables[L"NETHASH619"] = CalculateNetPath(absoluteAppPath, 619);
        variables[L"NETHASH620"] = CalculateNetPath(absoluteAppPath, 620);
        variables[L"NETHASH621"] = CalculateNetPath(absoluteAppPath, 621);
        variables[L"NETHASH622"] = CalculateNetPath(absoluteAppPath, 622);
        variables[L"NETHASH623"] = CalculateNetPath(absoluteAppPath, 623);
        variables[L"NETHASH624"] = CalculateNetPath(absoluteAppPath, 624);
        variables[L"NETHASH625"] = CalculateNetPath(absoluteAppPath, 625);
        variables[L"NETHASH626"] = CalculateNetPath(absoluteAppPath, 626);
        variables[L"NETHASH627"] = CalculateNetPath(absoluteAppPath, 627);
        variables[L"NETHASH628"] = CalculateNetPath(absoluteAppPath, 628);
        variables[L"NETHASH629"] = CalculateNetPath(absoluteAppPath, 629);
        variables[L"NETHASH630"] = CalculateNetPath(absoluteAppPath, 630);
        variables[L"NETHASH631"] = CalculateNetPath(absoluteAppPath, 631);
        variables[L"NETHASH632"] = CalculateNetPath(absoluteAppPath, 632);
        variables[L"NETHASH633"] = CalculateNetPath(absoluteAppPath, 633);
        variables[L"NETHASH634"] = CalculateNetPath(absoluteAppPath, 634);
        variables[L"NETHASH635"] = CalculateNetPath(absoluteAppPath, 635);
        variables[L"NETHASH636"] = CalculateNetPath(absoluteAppPath, 636);
        variables[L"NETHASH637"] = CalculateNetPath(absoluteAppPath, 637);
        variables[L"NETHASH638"] = CalculateNetPath(absoluteAppPath, 638);
        variables[L"NETHASH639"] = CalculateNetPath(absoluteAppPath, 639);
        variables[L"NETHASH640"] = CalculateNetPath(absoluteAppPath, 640);
        variables[L"NETHASH641"] = CalculateNetPath(absoluteAppPath, 641);
        variables[L"NETHASH642"] = CalculateNetPath(absoluteAppPath, 642);
        variables[L"NETHASH643"] = CalculateNetPath(absoluteAppPath, 643);
        variables[L"NETHASH644"] = CalculateNetPath(absoluteAppPath, 644);
        variables[L"NETHASH645"] = CalculateNetPath(absoluteAppPath, 645);
        variables[L"NETHASH646"] = CalculateNetPath(absoluteAppPath, 646);
        variables[L"NETHASH647"] = CalculateNetPath(absoluteAppPath, 647);
        variables[L"NETHASH648"] = CalculateNetPath(absoluteAppPath, 648);
        variables[L"NETHASH649"] = CalculateNetPath(absoluteAppPath, 649);
        variables[L"NETHASH650"] = CalculateNetPath(absoluteAppPath, 650);
        variables[L"NETHASH651"] = CalculateNetPath(absoluteAppPath, 651);
        variables[L"NETHASH652"] = CalculateNetPath(absoluteAppPath, 652);
        variables[L"NETHASH653"] = CalculateNetPath(absoluteAppPath, 653);
        variables[L"NETHASH654"] = CalculateNetPath(absoluteAppPath, 654);
        variables[L"NETHASH655"] = CalculateNetPath(absoluteAppPath, 655);
        variables[L"NETHASH656"] = CalculateNetPath(absoluteAppPath, 656);
        variables[L"NETHASH657"] = CalculateNetPath(absoluteAppPath, 657);
        variables[L"NETHASH658"] = CalculateNetPath(absoluteAppPath, 658);
        variables[L"NETHASH659"] = CalculateNetPath(absoluteAppPath, 659);
        variables[L"NETHASH660"] = CalculateNetPath(absoluteAppPath, 660);
        variables[L"NETHASH661"] = CalculateNetPath(absoluteAppPath, 661);
        variables[L"NETHASH662"] = CalculateNetPath(absoluteAppPath, 662);
        variables[L"NETHASH663"] = CalculateNetPath(absoluteAppPath, 663);
        variables[L"NETHASH664"] = CalculateNetPath(absoluteAppPath, 664);
        variables[L"NETHASH665"] = CalculateNetPath(absoluteAppPath, 665);
        variables[L"NETHASH666"] = CalculateNetPath(absoluteAppPath, 666);
        variables[L"NETHASH667"] = CalculateNetPath(absoluteAppPath, 667);
        variables[L"NETHASH668"] = CalculateNetPath(absoluteAppPath, 668);
        variables[L"NETHASH669"] = CalculateNetPath(absoluteAppPath, 669);
        variables[L"NETHASH670"] = CalculateNetPath(absoluteAppPath, 670);
        variables[L"NETHASH671"] = CalculateNetPath(absoluteAppPath, 671);
        variables[L"NETHASH672"] = CalculateNetPath(absoluteAppPath, 672);
        variables[L"NETHASH673"] = CalculateNetPath(absoluteAppPath, 673);
        variables[L"NETHASH674"] = CalculateNetPath(absoluteAppPath, 674);
        variables[L"NETHASH675"] = CalculateNetPath(absoluteAppPath, 675);
        variables[L"NETHASH676"] = CalculateNetPath(absoluteAppPath, 676);
        variables[L"NETHASH677"] = CalculateNetPath(absoluteAppPath, 677);
        variables[L"NETHASH678"] = CalculateNetPath(absoluteAppPath, 678);
        variables[L"NETHASH679"] = CalculateNetPath(absoluteAppPath, 679);
        variables[L"NETHASH680"] = CalculateNetPath(absoluteAppPath, 680);
        variables[L"NETHASH681"] = CalculateNetPath(absoluteAppPath, 681);
        variables[L"NETHASH682"] = CalculateNetPath(absoluteAppPath, 682);
        variables[L"NETHASH683"] = CalculateNetPath(absoluteAppPath, 683);
        variables[L"NETHASH684"] = CalculateNetPath(absoluteAppPath, 684);
        variables[L"NETHASH685"] = CalculateNetPath(absoluteAppPath, 685);
        variables[L"NETHASH686"] = CalculateNetPath(absoluteAppPath, 686);
        variables[L"NETHASH687"] = CalculateNetPath(absoluteAppPath, 687);
        variables[L"NETHASH688"] = CalculateNetPath(absoluteAppPath, 688);
        variables[L"NETHASH689"] = CalculateNetPath(absoluteAppPath, 689);
        variables[L"NETHASH690"] = CalculateNetPath(absoluteAppPath, 690);
        variables[L"NETHASH691"] = CalculateNetPath(absoluteAppPath, 691);
        variables[L"NETHASH692"] = CalculateNetPath(absoluteAppPath, 692);
        variables[L"NETHASH693"] = CalculateNetPath(absoluteAppPath, 693);
        variables[L"NETHASH694"] = CalculateNetPath(absoluteAppPath, 694);
        variables[L"NETHASH695"] = CalculateNetPath(absoluteAppPath, 695);
        variables[L"NETHASH696"] = CalculateNetPath(absoluteAppPath, 696);
        variables[L"NETHASH697"] = CalculateNetPath(absoluteAppPath, 697);
        variables[L"NETHASH698"] = CalculateNetPath(absoluteAppPath, 698);
        variables[L"NETHASH699"] = CalculateNetPath(absoluteAppPath, 699);
        variables[L"NETHASH700"] = CalculateNetPath(absoluteAppPath, 700);
        variables[L"NETHASH701"] = CalculateNetPath(absoluteAppPath, 701);
        variables[L"NETHASH702"] = CalculateNetPath(absoluteAppPath, 702);
        variables[L"NETHASH703"] = CalculateNetPath(absoluteAppPath, 703);
        variables[L"NETHASH704"] = CalculateNetPath(absoluteAppPath, 704);
        variables[L"NETHASH705"] = CalculateNetPath(absoluteAppPath, 705);
        variables[L"NETHASH706"] = CalculateNetPath(absoluteAppPath, 706);
        variables[L"NETHASH707"] = CalculateNetPath(absoluteAppPath, 707);
        variables[L"NETHASH708"] = CalculateNetPath(absoluteAppPath, 708);
        variables[L"NETHASH709"] = CalculateNetPath(absoluteAppPath, 709);
        variables[L"NETHASH710"] = CalculateNetPath(absoluteAppPath, 710);
        variables[L"NETHASH711"] = CalculateNetPath(absoluteAppPath, 711);
        variables[L"NETHASH712"] = CalculateNetPath(absoluteAppPath, 712);
        variables[L"NETHASH713"] = CalculateNetPath(absoluteAppPath, 713);
        variables[L"NETHASH714"] = CalculateNetPath(absoluteAppPath, 714);
        variables[L"NETHASH715"] = CalculateNetPath(absoluteAppPath, 715);
        variables[L"NETHASH716"] = CalculateNetPath(absoluteAppPath, 716);
        variables[L"NETHASH717"] = CalculateNetPath(absoluteAppPath, 717);
        variables[L"NETHASH718"] = CalculateNetPath(absoluteAppPath, 718);
        variables[L"NETHASH719"] = CalculateNetPath(absoluteAppPath, 719);
        variables[L"NETHASH720"] = CalculateNetPath(absoluteAppPath, 720);
        variables[L"NETHASH721"] = CalculateNetPath(absoluteAppPath, 721);
        variables[L"NETHASH722"] = CalculateNetPath(absoluteAppPath, 722);
        variables[L"NETHASH723"] = CalculateNetPath(absoluteAppPath, 723);
        variables[L"NETHASH724"] = CalculateNetPath(absoluteAppPath, 724);
        variables[L"NETHASH725"] = CalculateNetPath(absoluteAppPath, 725);
        variables[L"NETHASH726"] = CalculateNetPath(absoluteAppPath, 726);
        variables[L"NETHASH727"] = CalculateNetPath(absoluteAppPath, 727);
        variables[L"NETHASH728"] = CalculateNetPath(absoluteAppPath, 728);
        variables[L"NETHASH729"] = CalculateNetPath(absoluteAppPath, 729);
        variables[L"NETHASH730"] = CalculateNetPath(absoluteAppPath, 730);
        variables[L"NETHASH731"] = CalculateNetPath(absoluteAppPath, 731);
        variables[L"NETHASH732"] = CalculateNetPath(absoluteAppPath, 732);
        variables[L"NETHASH733"] = CalculateNetPath(absoluteAppPath, 733);
        variables[L"NETHASH734"] = CalculateNetPath(absoluteAppPath, 734);
        variables[L"NETHASH735"] = CalculateNetPath(absoluteAppPath, 735);
        variables[L"NETHASH736"] = CalculateNetPath(absoluteAppPath, 736);
        variables[L"NETHASH737"] = CalculateNetPath(absoluteAppPath, 737);
        variables[L"NETHASH738"] = CalculateNetPath(absoluteAppPath, 738);
        variables[L"NETHASH739"] = CalculateNetPath(absoluteAppPath, 739);
        variables[L"NETHASH740"] = CalculateNetPath(absoluteAppPath, 740);
        variables[L"NETHASH741"] = CalculateNetPath(absoluteAppPath, 741);
        variables[L"NETHASH742"] = CalculateNetPath(absoluteAppPath, 742);
        variables[L"NETHASH743"] = CalculateNetPath(absoluteAppPath, 743);
        variables[L"NETHASH744"] = CalculateNetPath(absoluteAppPath, 744);
        variables[L"NETHASH745"] = CalculateNetPath(absoluteAppPath, 745);
        variables[L"NETHASH746"] = CalculateNetPath(absoluteAppPath, 746);
        variables[L"NETHASH747"] = CalculateNetPath(absoluteAppPath, 747);
        variables[L"NETHASH748"] = CalculateNetPath(absoluteAppPath, 748);
        variables[L"NETHASH749"] = CalculateNetPath(absoluteAppPath, 749);
        variables[L"NETHASH750"] = CalculateNetPath(absoluteAppPath, 750);
        variables[L"NETHASH751"] = CalculateNetPath(absoluteAppPath, 751);
        variables[L"NETHASH752"] = CalculateNetPath(absoluteAppPath, 752);
        variables[L"NETHASH753"] = CalculateNetPath(absoluteAppPath, 753);
        variables[L"NETHASH754"] = CalculateNetPath(absoluteAppPath, 754);
        variables[L"NETHASH755"] = CalculateNetPath(absoluteAppPath, 755);
        variables[L"NETHASH756"] = CalculateNetPath(absoluteAppPath, 756);
        variables[L"NETHASH757"] = CalculateNetPath(absoluteAppPath, 757);
        variables[L"NETHASH758"] = CalculateNetPath(absoluteAppPath, 758);
        variables[L"NETHASH759"] = CalculateNetPath(absoluteAppPath, 759);
        variables[L"NETHASH760"] = CalculateNetPath(absoluteAppPath, 760);
        variables[L"NETHASH761"] = CalculateNetPath(absoluteAppPath, 761);
        variables[L"NETHASH762"] = CalculateNetPath(absoluteAppPath, 762);
        variables[L"NETHASH763"] = CalculateNetPath(absoluteAppPath, 763);
        variables[L"NETHASH764"] = CalculateNetPath(absoluteAppPath, 764);
        variables[L"NETHASH765"] = CalculateNetPath(absoluteAppPath, 765);
        variables[L"NETHASH766"] = CalculateNetPath(absoluteAppPath, 766);
        variables[L"NETHASH767"] = CalculateNetPath(absoluteAppPath, 767);
        variables[L"NETHASH768"] = CalculateNetPath(absoluteAppPath, 768);
        variables[L"NETHASH769"] = CalculateNetPath(absoluteAppPath, 769);
        variables[L"NETHASH770"] = CalculateNetPath(absoluteAppPath, 770);
        variables[L"NETHASH771"] = CalculateNetPath(absoluteAppPath, 771);
        variables[L"NETHASH772"] = CalculateNetPath(absoluteAppPath, 772);
        variables[L"NETHASH773"] = CalculateNetPath(absoluteAppPath, 773);
        variables[L"NETHASH774"] = CalculateNetPath(absoluteAppPath, 774);
        variables[L"NETHASH775"] = CalculateNetPath(absoluteAppPath, 775);
        variables[L"NETHASH776"] = CalculateNetPath(absoluteAppPath, 776);
        variables[L"NETHASH777"] = CalculateNetPath(absoluteAppPath, 777);
        variables[L"NETHASH778"] = CalculateNetPath(absoluteAppPath, 778);
        variables[L"NETHASH779"] = CalculateNetPath(absoluteAppPath, 779);
        variables[L"NETHASH780"] = CalculateNetPath(absoluteAppPath, 780);
        variables[L"NETHASH781"] = CalculateNetPath(absoluteAppPath, 781);
        variables[L"NETHASH782"] = CalculateNetPath(absoluteAppPath, 782);
        variables[L"NETHASH783"] = CalculateNetPath(absoluteAppPath, 783);
        variables[L"NETHASH784"] = CalculateNetPath(absoluteAppPath, 784);
        variables[L"NETHASH785"] = CalculateNetPath(absoluteAppPath, 785);
        variables[L"NETHASH786"] = CalculateNetPath(absoluteAppPath, 786);
        variables[L"NETHASH787"] = CalculateNetPath(absoluteAppPath, 787);
        variables[L"NETHASH788"] = CalculateNetPath(absoluteAppPath, 788);
        variables[L"NETHASH789"] = CalculateNetPath(absoluteAppPath, 789);
        variables[L"NETHASH790"] = CalculateNetPath(absoluteAppPath, 790);
        variables[L"NETHASH791"] = CalculateNetPath(absoluteAppPath, 791);
        variables[L"NETHASH792"] = CalculateNetPath(absoluteAppPath, 792);
        variables[L"NETHASH793"] = CalculateNetPath(absoluteAppPath, 793);
        variables[L"NETHASH794"] = CalculateNetPath(absoluteAppPath, 794);
        variables[L"NETHASH795"] = CalculateNetPath(absoluteAppPath, 795);
        variables[L"NETHASH796"] = CalculateNetPath(absoluteAppPath, 796);
        variables[L"NETHASH797"] = CalculateNetPath(absoluteAppPath, 797);
        variables[L"NETHASH798"] = CalculateNetPath(absoluteAppPath, 798);
        variables[L"NETHASH799"] = CalculateNetPath(absoluteAppPath, 799);
        variables[L"NETHASH800"] = CalculateNetPath(absoluteAppPath, 800);
        variables[L"NETHASH801"] = CalculateNetPath(absoluteAppPath, 801);
        variables[L"NETHASH802"] = CalculateNetPath(absoluteAppPath, 802);
        variables[L"NETHASH803"] = CalculateNetPath(absoluteAppPath, 803);
        variables[L"NETHASH804"] = CalculateNetPath(absoluteAppPath, 804);
        variables[L"NETHASH805"] = CalculateNetPath(absoluteAppPath, 805);
        variables[L"NETHASH806"] = CalculateNetPath(absoluteAppPath, 806);
        variables[L"NETHASH807"] = CalculateNetPath(absoluteAppPath, 807);
        variables[L"NETHASH808"] = CalculateNetPath(absoluteAppPath, 808);
        variables[L"NETHASH809"] = CalculateNetPath(absoluteAppPath, 809);
        variables[L"NETHASH810"] = CalculateNetPath(absoluteAppPath, 810);
        variables[L"NETHASH811"] = CalculateNetPath(absoluteAppPath, 811);
        variables[L"NETHASH812"] = CalculateNetPath(absoluteAppPath, 812);
        variables[L"NETHASH813"] = CalculateNetPath(absoluteAppPath, 813);
        variables[L"NETHASH814"] = CalculateNetPath(absoluteAppPath, 814);
        variables[L"NETHASH815"] = CalculateNetPath(absoluteAppPath, 815);
        variables[L"NETHASH816"] = CalculateNetPath(absoluteAppPath, 816);
        variables[L"NETHASH817"] = CalculateNetPath(absoluteAppPath, 817);
        variables[L"NETHASH818"] = CalculateNetPath(absoluteAppPath, 818);
        variables[L"NETHASH819"] = CalculateNetPath(absoluteAppPath, 819);
        variables[L"NETHASH820"] = CalculateNetPath(absoluteAppPath, 820);
        variables[L"NETHASH821"] = CalculateNetPath(absoluteAppPath, 821);
        variables[L"NETHASH822"] = CalculateNetPath(absoluteAppPath, 822);
        variables[L"NETHASH823"] = CalculateNetPath(absoluteAppPath, 823);
        variables[L"NETHASH824"] = CalculateNetPath(absoluteAppPath, 824);
        variables[L"NETHASH825"] = CalculateNetPath(absoluteAppPath, 825);
        variables[L"NETHASH826"] = CalculateNetPath(absoluteAppPath, 826);
        variables[L"NETHASH827"] = CalculateNetPath(absoluteAppPath, 827);
        variables[L"NETHASH828"] = CalculateNetPath(absoluteAppPath, 828);
        variables[L"NETHASH829"] = CalculateNetPath(absoluteAppPath, 829);
        variables[L"NETHASH830"] = CalculateNetPath(absoluteAppPath, 830);
        variables[L"NETHASH831"] = CalculateNetPath(absoluteAppPath, 831);
        variables[L"NETHASH832"] = CalculateNetPath(absoluteAppPath, 832);
        variables[L"NETHASH833"] = CalculateNetPath(absoluteAppPath, 833);
        variables[L"NETHASH834"] = CalculateNetPath(absoluteAppPath, 834);
        variables[L"NETHASH835"] = CalculateNetPath(absoluteAppPath, 835);
        variables[L"NETHASH836"] = CalculateNetPath(absoluteAppPath, 836);
        variables[L"NETHASH837"] = CalculateNetPath(absoluteAppPath, 837);
        variables[L"NETHASH838"] = CalculateNetPath(absoluteAppPath, 838);
        variables[L"NETHASH839"] = CalculateNetPath(absoluteAppPath, 839);
        variables[L"NETHASH840"] = CalculateNetPath(absoluteAppPath, 840);
        variables[L"NETHASH841"] = CalculateNetPath(absoluteAppPath, 841);
        variables[L"NETHASH842"] = CalculateNetPath(absoluteAppPath, 842);
        variables[L"NETHASH843"] = CalculateNetPath(absoluteAppPath, 843);
        variables[L"NETHASH844"] = CalculateNetPath(absoluteAppPath, 844);
        variables[L"NETHASH845"] = CalculateNetPath(absoluteAppPath, 845);
        variables[L"NETHASH846"] = CalculateNetPath(absoluteAppPath, 846);
        variables[L"NETHASH847"] = CalculateNetPath(absoluteAppPath, 847);
        variables[L"NETHASH848"] = CalculateNetPath(absoluteAppPath, 848);
        variables[L"NETHASH849"] = CalculateNetPath(absoluteAppPath, 849);
        variables[L"NETHASH850"] = CalculateNetPath(absoluteAppPath, 850);
        variables[L"NETHASH851"] = CalculateNetPath(absoluteAppPath, 851);
        variables[L"NETHASH852"] = CalculateNetPath(absoluteAppPath, 852);
        variables[L"NETHASH853"] = CalculateNetPath(absoluteAppPath, 853);
        variables[L"NETHASH854"] = CalculateNetPath(absoluteAppPath, 854);
        variables[L"NETHASH855"] = CalculateNetPath(absoluteAppPath, 855);
        variables[L"NETHASH856"] = CalculateNetPath(absoluteAppPath, 856);
        variables[L"NETHASH857"] = CalculateNetPath(absoluteAppPath, 857);
        variables[L"NETHASH858"] = CalculateNetPath(absoluteAppPath, 858);
        variables[L"NETHASH859"] = CalculateNetPath(absoluteAppPath, 859);
        variables[L"NETHASH860"] = CalculateNetPath(absoluteAppPath, 860);
        variables[L"NETHASH861"] = CalculateNetPath(absoluteAppPath, 861);
        variables[L"NETHASH862"] = CalculateNetPath(absoluteAppPath, 862);
        variables[L"NETHASH863"] = CalculateNetPath(absoluteAppPath, 863);
        variables[L"NETHASH864"] = CalculateNetPath(absoluteAppPath, 864);
        variables[L"NETHASH865"] = CalculateNetPath(absoluteAppPath, 865);
        variables[L"NETHASH866"] = CalculateNetPath(absoluteAppPath, 866);
        variables[L"NETHASH867"] = CalculateNetPath(absoluteAppPath, 867);
        variables[L"NETHASH868"] = CalculateNetPath(absoluteAppPath, 868);
        variables[L"NETHASH869"] = CalculateNetPath(absoluteAppPath, 869);
        variables[L"NETHASH870"] = CalculateNetPath(absoluteAppPath, 870);
        variables[L"NETHASH871"] = CalculateNetPath(absoluteAppPath, 871);
        variables[L"NETHASH872"] = CalculateNetPath(absoluteAppPath, 872);
        variables[L"NETHASH873"] = CalculateNetPath(absoluteAppPath, 873);
        variables[L"NETHASH874"] = CalculateNetPath(absoluteAppPath, 874);
        variables[L"NETHASH875"] = CalculateNetPath(absoluteAppPath, 875);
        variables[L"NETHASH876"] = CalculateNetPath(absoluteAppPath, 876);
        variables[L"NETHASH877"] = CalculateNetPath(absoluteAppPath, 877);
        variables[L"NETHASH878"] = CalculateNetPath(absoluteAppPath, 878);
        variables[L"NETHASH879"] = CalculateNetPath(absoluteAppPath, 879);
        variables[L"NETHASH880"] = CalculateNetPath(absoluteAppPath, 880);
        variables[L"NETHASH881"] = CalculateNetPath(absoluteAppPath, 881);
        variables[L"NETHASH882"] = CalculateNetPath(absoluteAppPath, 882);
        variables[L"NETHASH883"] = CalculateNetPath(absoluteAppPath, 883);
        variables[L"NETHASH884"] = CalculateNetPath(absoluteAppPath, 884);
        variables[L"NETHASH885"] = CalculateNetPath(absoluteAppPath, 885);
        variables[L"NETHASH886"] = CalculateNetPath(absoluteAppPath, 886);
        variables[L"NETHASH887"] = CalculateNetPath(absoluteAppPath, 887);
        variables[L"NETHASH888"] = CalculateNetPath(absoluteAppPath, 888);
        variables[L"NETHASH889"] = CalculateNetPath(absoluteAppPath, 889);
        variables[L"NETHASH890"] = CalculateNetPath(absoluteAppPath, 890);
        variables[L"NETHASH891"] = CalculateNetPath(absoluteAppPath, 891);
        variables[L"NETHASH892"] = CalculateNetPath(absoluteAppPath, 892);
        variables[L"NETHASH893"] = CalculateNetPath(absoluteAppPath, 893);
        variables[L"NETHASH894"] = CalculateNetPath(absoluteAppPath, 894);
        variables[L"NETHASH895"] = CalculateNetPath(absoluteAppPath, 895);
        variables[L"NETHASH896"] = CalculateNetPath(absoluteAppPath, 896);
        variables[L"NETHASH897"] = CalculateNetPath(absoluteAppPath, 897);
        variables[L"NETHASH898"] = CalculateNetPath(absoluteAppPath, 898);
        variables[L"NETHASH899"] = CalculateNetPath(absoluteAppPath, 899);
        variables[L"NETHASH900"] = CalculateNetPath(absoluteAppPath, 900);
        variables[L"NETHASH901"] = CalculateNetPath(absoluteAppPath, 901);
        variables[L"NETHASH902"] = CalculateNetPath(absoluteAppPath, 902);
        variables[L"NETHASH903"] = CalculateNetPath(absoluteAppPath, 903);
        variables[L"NETHASH904"] = CalculateNetPath(absoluteAppPath, 904);
        variables[L"NETHASH905"] = CalculateNetPath(absoluteAppPath, 905);
        variables[L"NETHASH906"] = CalculateNetPath(absoluteAppPath, 906);
        variables[L"NETHASH907"] = CalculateNetPath(absoluteAppPath, 907);
        variables[L"NETHASH908"] = CalculateNetPath(absoluteAppPath, 908);
        variables[L"NETHASH909"] = CalculateNetPath(absoluteAppPath, 909);
        variables[L"NETHASH910"] = CalculateNetPath(absoluteAppPath, 910);
        variables[L"NETHASH911"] = CalculateNetPath(absoluteAppPath, 911);
        variables[L"NETHASH912"] = CalculateNetPath(absoluteAppPath, 912);
        variables[L"NETHASH913"] = CalculateNetPath(absoluteAppPath, 913);
        variables[L"NETHASH914"] = CalculateNetPath(absoluteAppPath, 914);
        variables[L"NETHASH915"] = CalculateNetPath(absoluteAppPath, 915);
        variables[L"NETHASH916"] = CalculateNetPath(absoluteAppPath, 916);
        variables[L"NETHASH917"] = CalculateNetPath(absoluteAppPath, 917);
        variables[L"NETHASH918"] = CalculateNetPath(absoluteAppPath, 918);
        variables[L"NETHASH919"] = CalculateNetPath(absoluteAppPath, 919);
        variables[L"NETHASH920"] = CalculateNetPath(absoluteAppPath, 920);
        variables[L"NETHASH921"] = CalculateNetPath(absoluteAppPath, 921);
        variables[L"NETHASH922"] = CalculateNetPath(absoluteAppPath, 922);
        variables[L"NETHASH923"] = CalculateNetPath(absoluteAppPath, 923);
        variables[L"NETHASH924"] = CalculateNetPath(absoluteAppPath, 924);
        variables[L"NETHASH925"] = CalculateNetPath(absoluteAppPath, 925);
        variables[L"NETHASH926"] = CalculateNetPath(absoluteAppPath, 926);
        variables[L"NETHASH927"] = CalculateNetPath(absoluteAppPath, 927);
        variables[L"NETHASH928"] = CalculateNetPath(absoluteAppPath, 928);
        variables[L"NETHASH929"] = CalculateNetPath(absoluteAppPath, 929);
        variables[L"NETHASH930"] = CalculateNetPath(absoluteAppPath, 930);
        variables[L"NETHASH931"] = CalculateNetPath(absoluteAppPath, 931);
        variables[L"NETHASH932"] = CalculateNetPath(absoluteAppPath, 932);
        variables[L"NETHASH933"] = CalculateNetPath(absoluteAppPath, 933);
        variables[L"NETHASH934"] = CalculateNetPath(absoluteAppPath, 934);
        variables[L"NETHASH935"] = CalculateNetPath(absoluteAppPath, 935);
        variables[L"NETHASH936"] = CalculateNetPath(absoluteAppPath, 936);
        variables[L"NETHASH937"] = CalculateNetPath(absoluteAppPath, 937);
        variables[L"NETHASH938"] = CalculateNetPath(absoluteAppPath, 938);
        variables[L"NETHASH939"] = CalculateNetPath(absoluteAppPath, 939);
        variables[L"NETHASH940"] = CalculateNetPath(absoluteAppPath, 940);
        variables[L"NETHASH941"] = CalculateNetPath(absoluteAppPath, 941);
        variables[L"NETHASH942"] = CalculateNetPath(absoluteAppPath, 942);
        variables[L"NETHASH943"] = CalculateNetPath(absoluteAppPath, 943);
        variables[L"NETHASH944"] = CalculateNetPath(absoluteAppPath, 944);
        variables[L"NETHASH945"] = CalculateNetPath(absoluteAppPath, 945);
        variables[L"NETHASH946"] = CalculateNetPath(absoluteAppPath, 946);
        variables[L"NETHASH947"] = CalculateNetPath(absoluteAppPath, 947);
        variables[L"NETHASH948"] = CalculateNetPath(absoluteAppPath, 948);
        variables[L"NETHASH949"] = CalculateNetPath(absoluteAppPath, 949);
        variables[L"NETHASH950"] = CalculateNetPath(absoluteAppPath, 950);
        variables[L"NETHASH951"] = CalculateNetPath(absoluteAppPath, 951);
        variables[L"NETHASH952"] = CalculateNetPath(absoluteAppPath, 952);
        variables[L"NETHASH953"] = CalculateNetPath(absoluteAppPath, 953);
        variables[L"NETHASH954"] = CalculateNetPath(absoluteAppPath, 954);
        variables[L"NETHASH955"] = CalculateNetPath(absoluteAppPath, 955);
        variables[L"NETHASH956"] = CalculateNetPath(absoluteAppPath, 956);
        variables[L"NETHASH957"] = CalculateNetPath(absoluteAppPath, 957);
        variables[L"NETHASH958"] = CalculateNetPath(absoluteAppPath, 958);
        variables[L"NETHASH959"] = CalculateNetPath(absoluteAppPath, 959);
